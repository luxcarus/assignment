...
Datatable.prototype.init = function () {
  ...
  this.run.targetEl = document.querySelector(this.opts.selector)
  ...
  this.run.targetEl.appendChild(layer1)
  this.run.targetEl.appendChild(layer2)
  this.run.targetEl.appendChild(layer3)
...
Datatable.prototype.createSearch = function () {
  ...
  input.addEventListener('keyup', function (e) {
    $this.run.searchWord = e.target.value
    $this.renderContent()
  })
...
Datatable.prototype.renderContent = function (reqPage, start, end, sortBy) {
  ...
  searchWord = this.run.searchWord === null ? '' : this.run.searchWord
  ...
  if (!sortBy) {
    var ths = document.querySelectorAll('.sorting'),
        name = ths[0].getAttribute('tt-label')
    sortBy = {by: 'asc', name}
  }
